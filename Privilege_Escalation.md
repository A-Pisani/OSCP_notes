...
4. On Kali, we can use the winexe command to spawn a shell using these credentials:
    ...
    ```
#### Saved Creds
Windows has a runas command which allows users to run commands with the privileges of other users. This usually requires the knowledge of the other user’s password.
...
#### Access Tokens
Access Tokens are special objects in Windows which store a user’sidentity and privileges.
...
- Linux: 
    ...
    ```
💥💥 In *nix look for unusual users (UID > 1000).  
💥💥 Can we SSH using non "default" users? Check the file `/etc/ssh/sshd_config`) to bruteforce the password.
...
- Linux: 
    ...
    ```
💥 If there is a service listed that wasn't detected using Nmap, it's a good sign there's a firewall rule blocking it.
💥 Some services (eg. MySQL, TCP 3306) use the loopback interface (127.0.0.1).
...
- Windows
    ```cmd
    PS C:\Users\student> driverquery.exe /v /fo csv | ConvertFrom-CSV | Select-Object ‘Display Name’, ‘Start Mode’, Path                REM Listing loaded drivers
...
⚠️ Kernel exploits may often be unstable and may be one-shot or cause a system cras. Use it as a last resource!
...
Remote users can mount shares, access, create, modify files. By default, created files inherit the **remote** user’s id and group id
(as owner and group respectively), even if they don’t exist on the NFS server.
#### Root Squashing
